...
-- Update any existing database functions that might reference is_influencer
-- The check_influencer_id function needs to be updated to work without is_influencer
DROP FUNCTION IF EXISTS public.check_influencer_id();

-- Create a new version that considers users as influencers by default (unless they are agencies or credit cards)
